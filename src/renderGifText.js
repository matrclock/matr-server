...
const DEFAULT_BG = BACKGROUND_COLOR;
const DEFAULT_FG = FOREGROUND_COLOR;
...
function drawText(ctx, font, text, x, y, color) {
  ctx.fillStyle = color;
  let cursorX = x;
...
export async function renderTextGif(textItems, delay = 1000, { backgroundColor = DEFAULT_BG, foregroundColor = DEFAULT_FG } = {}) {
  const fontCache = {};
  const img = pureimage.make(64, 32);
  const ctx = img.getContext('2d');
  clearCanvas(ctx, backgroundColor);

  for (const { content, fontName, x = 0, y = 0, color } of textItems) {
    if (!fontCache[fontName]) fontCache[fontName] = await loadFont(fontName);
    drawText(ctx, fontCache[fontName], content, x, y, color || foregroundColor);
  }

  return makeGifFrame(img, delay);
}

export async function renderHorizontalScrollingTextGif({
  content, fontName, y = 0, delay = 100, pixelsPerFrame = 1,
  color, backgroundColor = DEFAULT_BG, foregroundColor = DEFAULT_FG
}) {
  const font = await loadFont(fontName);
  const textColor = color || foregroundColor;

  let textWidth = 0;
  for (const ch of content) textWidth += font[ch] ? font[ch].width + 1 : 4;
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    const ctx = img.getContext('2d');
    clearCanvas(ctx, backgroundColor);
    drawText(ctx, font, content, 64 - offset, y, textColor);
...
export async function renderVerticalScrollingTextGif({
  content, fontName, delay = 100, pixelsPerFrame = 1, lineSpacing = 1,
  color, backgroundColor = DEFAULT_BG, foregroundColor = DEFAULT_FG
}) {
  const font = await loadFont(fontName);
  const textColor = color || foregroundColor;
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    lines.forEach((line, idx) => {
      ...
      drawText(ctx, font, line, 0, yPos, textColor);
    });
    frames.push(makeGifFrame(img, delay));
  }
...
export async function renderMultilineHorizontalScrollGif({
  textItems, delay = 100,
  backgroundColor = DEFAULT_BG, foregroundColor = DEFAULT_FG
}) {
  const fontCache = {};
  for (const { fontName } of textItems) {
    if (!fontCache[fontName]) fontCache[fontName] = await loadFont(fontName);
  }

  const totalFrames = Math.max(...textItems.map(({ content, fontName, pixelsPerFrame = 1 }) => {
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    for (const { content, fontName, y = 0, pixelsPerFrame = 1, color } of textItems) {
      const font = fontCache[fontName];
      const offset = i * pixelsPerFrame;
      drawText(ctx, font, content, 64 - offset, y, color || foregroundColor);
    }
