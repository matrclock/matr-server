...
async function fetchWeatherData(forecastDay) {
...
async function summarizeWithChatGPT(weatherData) {
  ...
  const userPrompt = `Summarize the weather at both 9am and 1pm on ${date} from this data:\n\n${JSON.stringify(weatherData, null, 2)}\n\nYou're forecasting here, so avoid definite articles like "is". Be sure to include the temperature, wind speed, wind gusts, wind direction, and precipitation probability.`;
...
export async function weather() {
  ...
  if (cached && (Date.now() - cached.timestamp) < CACHE_TTL) {
    console.log("Returning cached weather summary.");
    return cached.summary;
  }

  try {
    const rawWeather = await fetchWeatherData(forecastDay);
    const relevant = extractRelevantData(rawWeather, hoursToSummarize);
    const summary = await summarizeWithChatGPT(relevant);

    cache.setKey(cacheKey, { summary, timestamp: Date.now() });
    cache.save();
